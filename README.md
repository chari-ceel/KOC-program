...
当前统一对话页已经采用真实前后端连接，不再由前端伪造 Agent 结果：

- 登录用户：消息内容、当前阶段、右侧摘要、完成状态、下方按钮、内容草稿标题都以后端 `/api/agent/chat` 和 `/api/agent/conversations` 返回为准。
- 游客用户：仍保留本地“一次免费人设生成”限制；生成也调用真实后端，但热门追踪、内容撰写和保存需要登录。
- 前端 `localStorage` 只保留当前激活对话 ID、游客限制和必要 UI 状态，不作为流程摘要、模块完成状态或草稿内容的数据源。
- 前端新增 `frontend/app/api/agent/[...path]/route.ts` 作为真实后端代理，统一把 `/api/agent/*` 转发到 backend，并给 Agent/模型生成链路预留 180 秒超时，不返回假数据。
- 人设阶段的问题由后端动态下发，展示在助手气泡内，点击后只会预填输入框，不会直接伪造发送。
- 人设未完成前，右侧摘要和会话标题不会提前展示半成品总结；只有后端确认后才进入下一步。
...
用户输入
  ...
  -> 后端保存对话消息和模块记忆
  -> 返回 assistant_message、current_step、summary、memory_refs、actions、question_blocks、readiness、copy_payload
```
